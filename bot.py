...
class LangameAPI:
    ...
    async def get_products_list(self) -> Dict:
        return await self._request("/public_api/products/list")

    async def get_products_expense_page(self, date_from: str, date_to: str, page: int = 1) -> Dict:
        params = {"date_from": date_from, "date_to": date_to, "page": page}
        return await self._request("/public_api/products/expense", params=params)

    async def get_all_products_expense(self, date_from: str, date_to: str) -> list:
        first_page = await self.get_products_expense_page(date_from, date_to, 1)
        if not first_page.get("status"):
            return []
        all_items = first_page.get("data", [])
        total_pages = first_page.get("total_pages", 1)
        for page in range(2, total_pages + 1):
            data = await self.get_products_expense_page(date_from, date_to, page)
            if data.get("status"):
...
async def get_stats_for_period(date_from: datetime, date_to: datetime) -> Dict:
    ...
    products_catalog = await api.get_products_list()
    goods_map = {}
    if products_catalog.get("status"):
        for item in products_catalog.get("data", []):
            goods_map[item["id"]] = item.get("name", f"Товар {item['id']}")

    sales = await api.get_all_products_expense(date_from_str, date_to_str)
    ...
    for sale in sales:
        ...
        product_name = goods_map.get(goods_id, f"Товар #{goods_id}")

        count = safe_float(sale.get("count", 0))
